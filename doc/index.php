...
/*
 *	PATH utilizzato in funzione di eliminazione
 */
...
//
$_CONFIG['users'] = array(array("user", "1238903nd213", "user"), array("admin", "admin", "admin"));
...
// INSERISCI IL NUMERO DI FILIALE QUI
//$_CONFIG['starting_dir'] = "51/";
...
$_IMAGES["del"] = "";
	...
	function run($location)
	{
		...
		if(isset($_GET['del'])){
			if(GateKeeper::isAccessAllowed() && GateKeeper::isDeleteAllowed()){
				...
				$path = EncodeExplorer::getConfig("basedir");
...
class File
{
	...
	function File($name, $location)
	{
		...
		$this->name = $name;
		$this->location = $location;
		$this->type = File::getFileType($this->location->GetFullPrefix().$this->location->getDir(true, false, false, 0).$this->getName());
		$this->size = File::getFileSize($this->location->GetFullPrefix().$this->location->getDir(true, false, false, 0).$this->getName());
		$this->modTime = filemtime($this->location->GetFullPrefix().$this->location->getDir(true, false, false, 0).$this->getName());
	}
...
class Location
{
	...
	{
		return (strlen(EncodeExplorer::getConfig('basedir')) > 0?EncodeExplorer::getConfig('basedir'):dirname($_SERVER['SCRIPT_FILENAME']))."/";
	}
	...
	function readDir()
	{
		...
		if($open_dir = @opendir($this->location->getFullPath()))
		{
			...
			while ($object = readdir($open_dir))
			{
				if($object != "." && $object != "..") 
				{
					if(is_dir($this->location->GetFullPrefix().$this->location->getDir(true, false, false, 0)."/".$object))
					{
...
if($this->files)
{
	...
	foreach ($this->files as $file)
	{
		...
		print "<td class=\"name\">\n";
		print "\t\t<a href=\"?d=".base64_url_encode($file->location->getDir(false, false, false, 0).$file->getName())."\"";
...
if(!ImageServer::showImage() && !Logger::logQuery())
{
	...
	{
		//download
		$prefix = $location->GetFullPrefix();
		$file = base64_url_decode($_GET['d']);
		
		header('Content-Type: application/octet-stream');
		header("Content-disposition: attachment; filename=\"".basename($prefix.$file)."\""); 
		header("Content-Transfer-Encoding: Binary"); 
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');
		header('Content-Length: ' . filesize($prefix.$file));
		readfile($prefix.$file);
		exit;
	}
